...
def assimp_convert_to_obj(src_path: Path) -> Path:
    ...
    try:
        ...
        print(f"⚠️ Assimp conversion failed for {src_path}: {e}")
    return None
...
def load_mesh_try(paths):
    for path in paths:
        ...
        if ext not in SUPPORTED_EXTENSIONS:
            print(f"⚠️ Unsupported file type: {ext}, skipping {path}")
            continue

        if Path(path).stat().st_size == 0:
            print("⚠️ Empty file, skipping:", path)
            continue

        # 1️⃣ Try trimesh directly
        try:
        ...
        # 3️⃣ Try converting with Assimp
